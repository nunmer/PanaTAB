...
<form action="profiles" method="POST" autocomplete="off">
	<div class="row">
		...
		<div class="col-sm-2">
			...
			<select class="form-control"  style="height:44px; margin-top: 15px; margin-bottom: 15px;" name="gender" required>
	...
	<?php
			if (isset($_POST["search"])) {
				...
				if($keyword == NULL && $city == "all" && $gender=="all"){
					$query=mysqli_query($conn,"SELECT * FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id;");
					$counter = mysqli_query($conn, "SELECT COUNT(*) as Count FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id;");
				}else if($keyword == NULL && $city == "all" && $gender != "all"){
					$query=mysqli_query($conn,"SELECT * FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `gender`='$gender';");
					$counter = mysqli_query($conn, "SELECT COUNT(*) as Count FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `gender`='$gender';");
				}else if($keyword == NULL && $city != "all" && $gender == "all"){
					$query=mysqli_query($conn,"SELECT * FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `city`='$city';");
					$counter = mysqli_query($conn, "SELECT COUNT(*) as Count FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `city`='$city';");
				}else if($keyword == NULL && $city != "all" && $gender != "all"){
					$query=mysqli_query($conn,"SELECT * FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `gender`='$gender' AND `city`='$city';");
					$counter = mysqli_query($conn, "SELECT COUNT(*) as Count FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `gender`='$gender' AND `city`='$city';");
				}else if($keyword != NULL && $city == "all" && $gender == "all"){
					$query=mysqli_query($conn,"SELECT * FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `username` = '$keyword';");
					$counter = mysqli_query($conn, "SELECT COUNT(*) as Count FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `username` = '$keyword';");
				}else if($keyword != NULL && $city == "all" && $gender != "all"){
					$query=mysqli_query($conn,"SELECT * FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `username` = '$keyword' AND `gender`='$gender';");
					$counter = mysqli_query($conn, "SELECT COUNT(*) as Count FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `username` = '$keyword' AND `gender`='$gender';");
				}else if($keyword != NULL && $city != "all" && $gender == "all"){
					$query=mysqli_query($conn,"SELECT * FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `username` = '$keyword' AND `city`='$city';");
					$counter = mysqli_query($conn, "SELECT COUNT(*) as Count FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `username` = '$keyword' AND `city`='$city';");
				}else if($keyword != NULL && $city != "all" && $gender != "all"){
					$query=mysqli_query($conn,"SELECT * FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `username` = '$keyword' AND `gender`='$gender' AND `city`='$city';");
					$counter = mysqli_query($conn, "SELECT COUNT(*) as Count FROM `tbl_profiles` LEFT JOIN `tbl_users` ON tbl_profiles.author=tbl_users.user_id WHERE `username` = '$keyword' AND `gender`='$gender' AND `city`='$city';");
				}
            ...
            <div class="row">
                ...
                <div class="col-sm-2">
                    <div class="section_title pb-15">
						<h4 style="color: grey;">Нашлось <?php echo $rows["Count"]; ?> профилей по данным фильтрам</h4>
						<?php 
							if($rows["Count"]==0){ echo "Опубликуйте своё объявление! Будьте первыми в этой категорий!";}
						?>
                    </div>
                </div>
            </div>
